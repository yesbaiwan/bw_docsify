...
- API 地址：https://api.zhubaiwan.xyz
  - 无法访问尝试在地址后添加 /v1，即 https://api.zhubaiwan.xyz/v1
- 模型列表：https://models.baiwan.de
- API 密钥：因被滥用现改为私信申请制（不是注册）！
  - 曾经申请过密钥但失效的，可以用旧密钥自动领取新的：https://baiwan-key.deno.dev （打不开用流量）
...
每日用量：[20-50]条（必填）
常用模型：[deepseek/deepseek-v4-pro、glm/glm-5...]（必填）
...
- 非官方，大多来逆向和自其他人提供的 API，可能不支持工具调用（tools），甚至可能不支持 temperature、top_p 等参数。
